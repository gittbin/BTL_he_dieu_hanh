...
icon.forEach((element) => {
  element.onclick = () => {
    ...
    if (Selections.style.height == "") {
      selection = Selections.querySelectorAll(".selection_item");
...
cpu.onclick = () => {
  for_process.style.display = "none";
...
interrupt.onclick = () => {
  ...
  // for_mouse.style.display="none"
  // for_change.style.display="none"
  for_interrupt.style.display = "flex";
};
...
function announcement(a, b, c = "") {
  b.querySelector(".announc").style.display = "flex";
  if (c == "") {
    if (a == 1) {
...
for_interrupt.querySelector(".commit").onclick = () => {
  check = 1;
  for_interrupt.querySelectorAll(".button").forEach((element) => {
    x = element.querySelector(".confirm");
    if (x != null) {
      if (element.querySelector(".common").value == "") {
        x.style.color = "red";
  ...
  if (check) {
    var params = new URLSearchParams();
    params.append("x", input_interrupt.value);
    console.log("http://127.0.0.1:8000/interrupt?" + params.toString());
    change_loading(1);
    fetch("http://127.0.0.1:8000/interrupt?" + params.toString(), {
      method: "GET",
      cache: "no-cache",
      credentials: "same-origin",
      headers: {
      ...
      .then((response) => {
        change_loading(0);
        if (response.result["announc"] == 1) {
          announcement(1, for_interrupt, response.result["infor"]);
        } else {
          announcement(0, for_interrupt, response.result["infor"]);
        }
      })
      .catch((error) => console.error("Error: ", error));
  }
...
b2 = function () {
  return () => {
    for_device.style.display = "none";
    for_process.style.display = "flex";
    for_device.removeEventListener("dblclick", handleClick);
  };
};
...
a2 = function (x) {
  return () => {
    ...
    // Tạo sự kiện mới và lưu trữ nó
    handleClick = b2();
    for_device.addEventListener("dblclick", handleClick);
  };
};
show.onclick = () => {
  for_process.style.display = "flex";
  ...
  fetch("http://127.0.0.1:8000/process", {
    ...
    .then((response) => {
      ...
      for (key in response) {
        ...
        div.addEventListener("dblclick", a2(response[key]));
        div.innerHTML = `<h3 class="text_in_block">${key}</h3>`;
        for_process.append(div);
      }
    })
    .catch((error) => console.error("Error: ", error));
};
